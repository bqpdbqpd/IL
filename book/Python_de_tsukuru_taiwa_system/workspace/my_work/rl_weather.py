...
# フレームから状態を表す文字列に変換
# place, date, type の順に値が埋まっていたら1，埋まっていなければ0
...
# open-prompt には聞きたいことをいくつかランダムに伝える
# tell-info によるシステム回答の内容が会っていたら goodbye をする
# tell-infoの内容が間違っていたら initialize をする
def next_user_da(sys_da, sys_conceptdic, intention):
    if sys_da == "ask-place":
        return "request-wearher", {"place": intention["place"]}
...
def next_system_da(frame):
    # 値がすべて埋まっていないと tell-info は発話できない
...
# 対話を成功するまで一回実行
# intention はユーザの意図， alpha は学習係数， gamma は割引率を表す
def run_dialogue(intention, alpha=0.1, gamma=0.9):
    ...
    while(True):
        ...
        maxval = da_lis[0][1]
        if da == "goodbye":
            # 成功した対話の後の状態は存在しないので maxval は 0
            maxval = 0
            # Q値を更新して対話を終わる
